	...
	*/
    ...
    function decodeTlds(encoded) {
        ...
        while (i < encoded.length) {
            ...
            if (popDigitCount > 0) {
                ...
                let popCount = parseInt(
                    encoded.substring(i, i + popDigitCount),
                    10
                );
    ...
    Options.prototype = {
        ...
        render(token) {
            ...
            const renderFn =
                this.get("render", null, token) || this.defaultRender;
	...
	*/
    ...
    function run(start, input, tokens) {
        ...
        while (cursor < len) {
            ...
            while (
                cursor < len &&
                !(secondState = state.go(tokens[cursor].t))
            ) {
    ...
    function registerPlugin(name, plugin) {
        if (typeof plugin !== "function") {
            throw new Error(
                `linkifyjs: Invalid plugin ${plugin} (expects function)`
            );
        }
    ...
    function find(str, type, opts) {
        ...
        if (type && typeof type === "object") {
            if (opts) {
                throw Error(
                    `linkifyjs: Invalid link type ${type}; must be a string`
                );
            }
            opts = type;
            type = null;
